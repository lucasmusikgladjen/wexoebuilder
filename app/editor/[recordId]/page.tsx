...
import { loadLandingPageState } from '@/lib/landing-page-loader';
...
export default async function EditLandingPage({ params }: Props) {
  ...
  if (!apiKey) {
    return <ErrorScreen title="Konfigurationsfel" message="AIRTABLE_API_KEY saknas i miljövariablerna." />;
  }

  let state: PageState;
  try {
    state = await loadLandingPageState(apiKey, recordId);
  } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white" style={{ fontFamily: 'var(--font-dm-sans)' }}>
      <div className="max-w-md text-center px-8">
        ...
        <Link href="/" className="inline-block text-sm text-gray-400 hover:text-gray-700 transition-colors">
          ← Tillbaka till sidor
        </Link>
      </div>
    </div>
